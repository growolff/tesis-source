...
CY_ISR(PM2_HA_INT) //interrupt on level change in PM2_HA signal
{
...
CY_ISR(SPD_COMMAND_INT)
{
    ...
    if(PM1.control_mode == 1 || PM1.control_mode == 2){
        MOTOR_sendSpeedCommand(1,(uint8)(fn_mapper_8b(PM1.spdPID_out,0,VEL_MAX,0,255)));
        //PM1_SPD_VDAC8_SetValue((uint8)(fn_mapper_8b(PM1.spdPID_out,0,9500,0,255)));
        //MOTOR_commandDriver(&PM1,PMF,(uint8)(fn_mapper_8b(PM1.spdPID_out,0,VEL_MAX,0,255)));
    }
    
    // Write speed command in motor PM2 VDAC
    MOTOR_setSpdRef(&PM2,PM2.ref_spd);
    if(PM2.control_mode == 1 || PM2.control_mode == 2){
        MOTOR_sendSpeedCommand(2,(uint8)(fn_mapper_8b(PM2.spdPID_out,0,VEL_MAX,0,255)));
        //PM2_SPD_VDAC8_SetValue((uint8)(fn_mapper_8b(PM2.spdPID_out,0,VEL_MAX,0,255)));
...
CY_ISR(TNS_COMMAND_INT)
{
    ...
    /*
    MOTOR_readCurrentTension(&PM2,2);
