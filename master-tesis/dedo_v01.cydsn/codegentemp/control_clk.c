...
/*******************************************************************************
* Function Name: control_clk_Start
********************************************************************************
*
* Summary:
...
void control_clk_Start(void) 
{
    ...
    control_clk_CLKEN |= control_clk_CLKEN_MASK;
	control_clk_CLKSTBY |= control_clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: control_clk_Stop
********************************************************************************
*
* Summary:
...
void control_clk_Stop(void) 
{
    ...
    control_clk_CLKEN &= (uint8)(~control_clk_CLKEN_MASK);
	control_clk_CLKSTBY &= (uint8)(~control_clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: control_clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((control_clk_CLKEN & control_clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = control_clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = control_clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* control_clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(control_clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(control_clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: control_clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: control_clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = control_clk_GetSourceRegister();
    uint16 oldDivider = control_clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = control_clk_CLKEN & control_clk_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((control_clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: control_clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(control_clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: control_clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    control_clk_MOD_SRC |= modeBitMask & (uint8)control_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: control_clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    control_clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(control_clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: control_clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return control_clk_MOD_SRC & (uint8)(control_clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: control_clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = control_clk_GetDividerRegister();
    uint8 oldSrc = control_clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        control_clk_MOD_SRC |= CYCLK_SSS;
        control_clk_MOD_SRC =
            (control_clk_MOD_SRC & (uint8)(~control_clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: control_clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return control_clk_MOD_SRC & control_clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: control_clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    control_clk_PHASE = clkPhase & control_clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: control_clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return control_clk_PHASE & control_clk_PHASE_MASK;
}
