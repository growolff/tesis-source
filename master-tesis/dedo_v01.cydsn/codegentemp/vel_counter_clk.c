...
/*******************************************************************************
* Function Name: vel_counter_clk_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void vel_counter_clk_Start(void) 
{
    /* Set the bit to enable the clock. */
    vel_counter_clk_CLKEN |= vel_counter_clk_CLKEN_MASK;
	vel_counter_clk_CLKSTBY |= vel_counter_clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: vel_counter_clk_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void vel_counter_clk_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    vel_counter_clk_CLKEN &= (uint8)(~vel_counter_clk_CLKEN_MASK);
	vel_counter_clk_CLKSTBY &= (uint8)(~vel_counter_clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: vel_counter_clk_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void vel_counter_clk_StopBlock(void) 
{
    if ((vel_counter_clk_CLKEN & vel_counter_clk_CLKEN_MASK) != 0u)
    {
...
#if defined(vel_counter_clk__CFG3)
        CLK_DIST_AMASK = vel_counter_clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = vel_counter_clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* vel_counter_clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(vel_counter_clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(vel_counter_clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: vel_counter_clk_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void vel_counter_clk_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        vel_counter_clk_CLKSTBY &= (uint8)(~vel_counter_clk_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: vel_counter_clk_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void vel_counter_clk_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = vel_counter_clk_GetSourceRegister();
    uint16 oldDivider = vel_counter_clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = vel_counter_clk_CLKEN & vel_counter_clk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(vel_counter_clk_DIV_PTR, clkDivider);
                vel_counter_clk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                vel_counter_clk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(vel_counter_clk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((vel_counter_clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: vel_counter_clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 vel_counter_clk_GetDividerRegister(void) 
{
    return CY_GET_REG16(vel_counter_clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: vel_counter_clk_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void vel_counter_clk_SetModeRegister(uint8 modeBitMask) 
{
    vel_counter_clk_MOD_SRC |= modeBitMask & (uint8)vel_counter_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: vel_counter_clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void vel_counter_clk_ClearModeRegister(uint8 modeBitMask) 
{
    vel_counter_clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(vel_counter_clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: vel_counter_clk_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 vel_counter_clk_GetModeRegister(void) 
{
    return vel_counter_clk_MOD_SRC & (uint8)(vel_counter_clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: vel_counter_clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void vel_counter_clk_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = vel_counter_clk_GetDividerRegister();
    uint8 oldSrc = vel_counter_clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        vel_counter_clk_MOD_SRC |= CYCLK_SSS;
        vel_counter_clk_MOD_SRC =
            (vel_counter_clk_MOD_SRC & (uint8)(~vel_counter_clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: vel_counter_clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 vel_counter_clk_GetSourceRegister(void) 
{
    return vel_counter_clk_MOD_SRC & vel_counter_clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: vel_counter_clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void vel_counter_clk_SetPhaseRegister(uint8 clkPhase) 
{
    vel_counter_clk_PHASE = clkPhase & vel_counter_clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: vel_counter_clk_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 vel_counter_clk_GetPhaseRegister(void) 
{
    return vel_counter_clk_PHASE & vel_counter_clk_PHASE_MASK;
}
